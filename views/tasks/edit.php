...
<div class="container">
    ...
    <form action="index.php?controller=task&action=update" method="post">
        ...
        <div class="form-group mb-3" >
            ...
            <textarea class="form-control" id="description" name="description" required><?= $data['task']['description'] ?></textarea>
        </div>

        <div class="form-group mb-3">
            <label for="priority">Priority</label>
            <input type="number" class="form-control" id="priority" name="priority" value="<?= $data['task']['priority'] ?>" required>
        </div>

        <div class="form-group mb-3">
            <label for="estimatedTime">Estimated Time</label>
            <input type="number" class="form-control" id="estimatedTime" name="estimatedTime" value="<?= $data['task']['estimatedTime'] ?>" required>
        </div>
        ...
        <div class="form-group mb-3">
            ...
            <select class="form-control" id="status" name="status" required>
                ...
                <option value="in_progress" <?= $data['task']['status'] == 'in_progress' ? 'selected' : '' ?>>In Progress</option>
                <option value="completed" <?= $data['task']['status'] == 'completed' ? 'selected' : '' ?>>Completed</option>
            </select>
        </div>
        ...
        <div class="form-group mb-3">
            ...
            <select class="form-control" id="sprintId" name="sprintId">
                <?php foreach ($data['sprints'] as $sprint): ?>
                    <option  value="<?= $sprint['id']; ?>" <?= $data['task']['sprintId'] == $sprint['id'] ? 'selected' : '' ?>><?= $sprint['scrumTeamId']." ".$sprint['name']; ?></option>
                <?php endforeach; ?>
            </select>
        </div>
